...
def generate_parsec_coordinates(samples, xte=1.0, io_flag=True): # 'x trailing edge'
    ...
    # Generate & solve equation systems
    upper_polynomial_coefficients, lower_polynomial_coefficients = generate_polynomial_coefficients(samples)

    sigstring = f'(6)->({N_XY_COORDINATES,2})'

    vec_upper_y_solutions = np.vectorize(upper_y_solutions, signature=sigstring)
    vec_lower_y_solutions = np.vectorize(lower_y_solutions, signature=sigstring)
...
def export_parsec_coordinates(upper_xy, lower_xy, io_flag):
    ...
    for index in range(upper_xy.shape[0]):
        ...
        # allows for filtering undesireable airfoil shapes
        ...
        is_invalid_airfoil = True if is_multimodal_airfoil or is_itersecting_airfoil or is_negative_upper_y else False 
        ...
        if not is_invalid_airfoil:
            ...
            # store coordinates in xfoil compatible format
            np.savetxt(f'airfoil_{index}.dat', stacked_xy, fmt='%f', delimiter=' ', newline='\n', header=f'airfoil_{index}', comments='') if io_flag else None

            # Calculate the surface area using the trapezoidal rule in a vectorized manner (done out of the box by using numpy functions)
